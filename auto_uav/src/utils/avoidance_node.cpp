...
void AvoidanceNode::init() {
    ...
    px4_param_sub_ =
        nh_.subscribe("/mavros/param/param_value", 1, &AvoidanceNode::px4ParamsCallback, this);
    mission_sub_ =
        nh_.subscribe("/mavros/mission/waypoints", 1, &AvoidanceNode::missionCallback, this);
    ...
    ros::TimerOptions statuslooptimer_options(
        ros::Duration(statusloop_dt_), boost::bind(&AvoidanceNode::statusLoopCallback, this, _1),
        &statusloop_queue_);
...
void AvoidanceNode::checkFailsafe(ros::Duration since_last_cloud, ros::Duration since_start,
                                  bool& hover) {
    ...
    while (!should_exit_) {
        bool is_param_not_initialized = true;
        {
            ...
            is_param_not_initialized =
                ...
                !std::isfinite(px4_.param_mpc_land_speed) ||
                !std::isfinite(px4_.param_nav_acc_rad) || !std::isfinite(px4_.param_mpc_acc_hor) ||
                !std::isfinite(px4_.param_mpc_jerk_max) ||
                !std::isfinite(px4_.param_mpc_acc_down_max) ||
                !std::isfinite(px4_.param_mpc_acc_up_max) ||
                !std::isfinite(px4_.param_mpc_z_vel_max_dn) ||
                !std::isfinite(px4_.param_mpc_z_vel_max_up) ||
    ...
    for (int index = 0; index < msg.waypoints.size(); index++) {
        if (msg.waypoints[index].is_current) {
            for (int i = index; i >= 0; i--) {
                if (msg.waypoints[i].command ==
                        static_cast<int>(MavCommand::MAV_CMD_DO_CHANGE_SPEED) &&
